...
interface RestaurantListInfiniteLoaderProps {
  hasNextPage: boolean;
...
export function RestaurantListInfiniteLoader({
  hasNextPage,
  isFetchingNextPage,
  fetchNextPage,
  shown,
  total,
}: RestaurantListInfiniteLoaderProps) {
  ...
  const showCount =
    shown != null && total != null && total > shown && hasNextPage;

  return (
    <div className="mt-8 flex flex-col items-center gap-2">
      {showCount && (
        <p className="text-sm text-muted-foreground">
          {t("restaurants.showingCount", { shown, total })}
        </p>
      )}
      <div
        ref={sentinelRef}
      ...
      >
        {hasNextPage && (
          ...
          >
            {isFetchingNextPage ? t("restaurants.loading") : t("restaurants.loadMore")}
          </Button>
        )}
      </div>
    </div>
