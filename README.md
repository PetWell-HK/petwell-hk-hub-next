...
- Forum HTTP: `NEXT_PUBLIC_PETWELL_API_URL` (production) + `NEXT_PUBLIC_PETWELL_API_URL_DEV` (localhost). `next dev` tries local `/api/health` then falls back to production.
